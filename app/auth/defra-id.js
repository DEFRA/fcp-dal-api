...
/**
 * Verifies the Defra ID token, eagerly, as part of context build. Verification is only attempted
 * when authContext.externalAuthHeader is set - a request authenticated another way (internal
 * email/service-account headers) carries no Defra ID token at all, so this resolves to undefined
 * without doing any work. When a header is present, a failed verification/decode throws
 * immediately from here (out of context build), rather than being deferred to crn()/orgId().
 *
