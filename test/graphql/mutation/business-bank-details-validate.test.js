...
describe('validateBusinessCustomerBankDetails', () => {
  ...
  test('returns BankDetailsMatched on a full match and does not submit', async () => {
    let validatedBody
    v1.post('/bank-change-service/v1/validate', (body) => {
      validatedBody = body
      return true
    }).reply(200, {
      status: 'MATCH',
    ...
    expect(result.data.validateBusinessCustomerBankDetails).toEqual({
  ...
  test('returns BankDetailsPartialMatch on a partial match and does not submit', async () => {
  ...
  test('returns BankDetailsLocked when no validation attempts remain', async () => {
    v1.post('/bank-change-service/v1/validate').reply(200, {
      status: 'FAILED',
      message: "Details don't match",
      attemptsRemaining: 0,
  ...
  test('returns NotFound when organisation has no FRN', async () => {
    ...
    expect(result.data).toEqual({ validateBusinessCustomerBankDetails: null })
